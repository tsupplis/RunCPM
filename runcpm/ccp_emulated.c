...
// Converts the PAR_FCB name to a number
static uint16_t ccp_fcb_to_num() {
	uint8_t ch;
	uint16_t n = 0;
	uint8_t pos = PAR_FCB + 1;
	while (1) {
		ch = ram_read(pos++);
		if (ch<'0' || ch>'9')
			break;
...
static void ccp_dir(void) {
	...
	dir_head[0] = ram_read(PAR_FCB) ? ram_read(PAR_FCB) + '@' : 'A';

	pal_puts("\r\n");
	if (!disk_search_first(PAR_FCB, 1)) {
		pal_puts((char*)dir_head);
		ccp_printfcb(GLB_TMP_FCB_ADDR, 0);
		fcount++; ccount++;
		while (!disk_search_next(PAR_FCB, 1)) {
			if (!ccount) {
				pal_puts("\r\n");
				pal_puts((char*)dir_head);
			} else {
	...
	if (pages < 256) {
		...
		if (ccp_bdos(F_MAKE, PAR_FCB)) {
			...
			if (ccp_bdos(F_OPEN, PAR_FCB)) {
				...
				pal_puts("\r\n");
				for (i = 0; i < pages; i++) {
					ccp_bdos(F_DMAOFF, dma);
					ccp_bdos(F_WRITE, PAR_FCB);
					dma += 128;
					ccp_bdos(C_WRITE, '.');
				}
				ccp_bdos(F_CLOSE, PAR_FCB);
			}
		}
	}
	return(error);
...
static uint8_t ccp_user(void) {
	...
	if (ccp_cur_user < 16) {
		ccp_bdos(F_USERNUM, ccp_cur_user);
		error = 0;
	}
	return(error);
}
...
static uint8_t ccp_lua(void) {
	...
	//uint16_t load_addr = DEF_LOAD;

	ram_write(CMD_FCB + 9, 'L');
	ram_write(CMD_FCB + 10, 'U');
...
static uint8_t ccp_ext(void) {
	...
	ram_write(CMD_FCB + 9, 'C');
	ram_write(CMD_FCB + 10, 'O');
	...
	if (found) {
		pal_puts("\r\n");
		ccp_bdos(F_DMAOFF, load_addr);
		while (!ccp_bdos(F_READ, CMD_FCB)) {
			load_addr += 128;
			ccp_bdos(F_DMAOFF, load_addr);
		}
		...
		load_addr = TRAMPOLINE;
		ram_write(load_addr, CALL);
		ram_write16(load_addr + 1, DEF_LOAD);
...
void ccp(void) {
	...
	ccp_s_flag = (uint8_t)ccp_bdos(DRV_ALLRESET, 0x0000);
	ccp_bdos(DRV_SET, ccp_cur_drive);
