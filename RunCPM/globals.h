...
#define GLB_BIOS_PAGE			(RAM_SIZE*1024 - 256)
...
#ifdef CCP_DR
#define GLB_CCP_NAME		"CCP-DR." STR(RAM_SIZE) "K"
...
//
#ifdef CCP_CCPZ
#define GLB_CCP_NAME		"CCP-CCPZ." STR(RAM_SIZE) "K"
...
//
#ifdef CCP_ZCPR2
#define GLB_CCP_NAME		"CCP-ZCP2." STR(RAM_SIZE) "K"
...
//
#ifdef CCP_ZCPR3
#define GLB_CCP_NAME		"CCP-ZCP3." STR(RAM_SIZE) "K"
...
//
#ifdef CCP_Z80
#define GLB_CCP_NAME		"CCP-Z80." STR(RAM_SIZE) "K"
#define GLB_CCP_VERSION	0x04
#define GLB_BATCH_FCB_ADDR	(GLB_CCP_ADDR + 0x79E)		// Position of the $$$.SUB fcb on this CCP
#define GLB_CCP_ADDR		(GLB_BDOS_JUMP_PAGE-0x0800)
#endif
//
#define GLB_CCP_BANNER		"\r\nRunCPM Version " EMULATOR_VERSION " (CP/M 2.2 " STR(RAM_SIZE) "K)\r\n"

/* Definition of global variables */
extern uint8_t	_glb_file_name[17];		// Current filename in host filesystem format
extern uint8_t	_glb_new_name[17];		// New filename in host filesystem format
extern uint8_t	_glb_fcb_name[13];		// Current filename in CP/M format
extern uint8_t	_glb_pattern[13];		// File matching pattern in CP/M format
extern uint16_t	_glb_dma_addr;	// Current dmaAddr
extern uint8_t	_glb_c_drive;			// Old selected drive
extern uint8_t	_glb_o_drive;			// Currently selected drive
extern uint8_t	_glb_user_code;		// Current user code
extern uint16_t	_glb_ro_vector;
extern uint16_t	_glb_login_vector;
